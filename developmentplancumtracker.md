...
| 0.9 | SignalR infrastructure (hubs, groups per branch) | §9 | 🟩 | | **Done — three hubs + per-tenant group scoping, all verified cross-client.** Hubs: (1) `QueueHub` `/hubs/queue` (board live-refresh on token issue/call-next, "Live" badge + auto-reconnect); (2) `AlertsHub` `/hubs/alerts` (emergency toast on triage registration); (3) `GpsHub` `/hubs/gps` (live ambulance tracking — `POST /api/ambulance/{id}/location`, coords validated, map x/y from config `Gps:Bounds`; mini-map of moving dots + position table + Simulate driver). **Per-tenant isolation:** `TenantHub` joins each connection to its resolved tenant's group (read from the connection's `HttpContext.RequestServices`, since the hub's own DI scope ≠ the request scope), and every broadcast targets `Clients.Group(tenant:{code})` — the real-time mirror of the per-tenant DB isolation (L1.9.2). **Verified:** a foreign-tenant (`X-Tenant`) GPS ping did **not** reach the DEV client (0 rows) while a same-tenant ping did (delivered); queue + emergency hubs unchanged (0 console errors). All broadcasts originate in the Api layer so CQRS handlers stay transport-agnostic |
...
| 5.6 | Ambulance Management & GPS Tracking (call logging, nearest dispatch, live GPS via SignalR) | §3.6 | 🟩 | | Dispatch nearest-available + arrive + list — verified; screen wired. **Live GPS via SignalR done** (`GpsHub`, 0.9) — control-room mini-map + live position table plot vehicles in real time; verified cross-client |
