...
| 0.9 | SignalR infrastructure (hubs, groups per branch) | §9 | 🟦 | | **Two hubs live** — (1) `QueueHub` `/hubs/queue`: `QueueController` pushes `queueChanged` on token issue/call-next, the wireframe Queue board live-refreshes with a "Live" badge + auto-reconnect; (2) **`AlertsHub` `/hubs/alerts`: `EmergencyController` pushes `emergencyAlert` when a triage is registered, and the app shell shows a hospital-wide toast on any open screen.** Both **verified cross-client** (separate client's action pushed to the browser with no refresh, 0 console errors). Remaining: per-branch groups + ambulance GPS hub |
