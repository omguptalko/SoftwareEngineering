...
| 0.9 | SignalR infrastructure (hubs, groups per branch) | §9 | 🟩 | | **Three hubs live & verified cross-client** — (1) `QueueHub` `/hubs/queue`: queue board live-refreshes on token issue/call-next ("Live" badge + auto-reconnect); (2) `AlertsHub` `/hubs/alerts`: hospital-wide emergency toast on triage registration; (3) **`GpsHub` `/hubs/gps`: live ambulance tracking — `POST /api/ambulance/{id}/location` (coords validated, map x/y normalised from config `Gps:Bounds`) broadcasts `ambulanceMoved`; the Ambulance screen plots moving dots on a mini-map + live position table, with a Simulate driver.** All pushed from the Api layer (handlers stay transport-agnostic). Each verified with a *separate* client driving the update (0 console errors). Refinement remaining: per-tenant/branch **group scoping** (broadcasts are currently hospital-wide) |
