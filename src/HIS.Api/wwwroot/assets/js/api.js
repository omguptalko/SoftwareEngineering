...
HIS.api = (function () {
  ...
  return {
    ...
    bloodRequests:   () => get('/api/bloodbank/requests'),
    addBloodStock:   (cmd) => post('/api/bloodbank/stock/add', cmd),
    issueBlood:      (requestId) => post(`/api/bloodbank/requests/${requestId}/issue`, {}),
    ...
    setPoStatus:     (poId, status) => post(`/api/inventory/purchase-orders/${poId}/status`, { status }),
    ...
    doctorMaster:    () => get('/api/masters/doctors'),
    saveDoctor:      (cmd) => post('/api/masters/doctors', cmd),
    setDoctorActive: (doctorId, isActive) => post('/api/masters/doctors/set-active', { doctorId, isActive }),
    ...
    claimGet:        (claimId) => get(`/api/claims/${claimId}`),
    reconcile:       (cmd) => post('/api/claims/reconcile', cmd),
    ...
    mortuaryList:    () => get('/api/mortuary'),
    admitBody:       (cmd) => post('/api/mortuary/admit', cmd),
    releaseBody:     (id) => post(`/api/mortuary/${id}/release`, {}),
    consentTemplates:() => get('/api/consent/templates'),
    consentCaptures: () => get('/api/consent/captures'),
    captureConsent:  (cmd) => post('/api/consent', cmd),
