...
| L1.8.5 | Write-aggregate repository cutover (big-bang) + retire single-DB factory + docs | 🟦 | **Enablers done** (M0004: master nav + `proc.usp_NextDocNo`; backfill nav). Decisions: D8 = app-side two-step joins; execution = full switch then verify. **Remaining (scoped):** switch ~23 repo classes across 8 files (master-plane: ClinicalRepositories, Phase23/4/9/10 master parts; FY-plane: Phase23/4/6/7/8/9/10), refactor **~15 cross-DB joins** (FY repos joining master Patient/Doctor/Payer) to two-step, FY numbering → 4-arg `usp_NextDocNo`+FyCode, pharmacy cross-plane dispense (FY-authoritative + best-effort master stock), route `AuditWriter` → tenant master `audit`, retire `SqlConnectionFactory`, verify ~40 endpoints. Must land as one coordinated change (dbo FKs). |
